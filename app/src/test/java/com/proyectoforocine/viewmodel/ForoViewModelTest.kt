...
class ForoViewModelTest {
    ...
    fun `validarYCrearTema should return false and set error when titulo is blank`() = runTest {
        ...
        // Then
        assertFalse(result)
        assertEquals("El título y el contenido no pueden estar vacíos", viewModel.crearTemaUiState.value.error)
    ...
    @Test
    fun `validarYCrearTema should return true and call repository when both fields are valid`() = runTest {
        // Given
